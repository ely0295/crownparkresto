...
if(isset($_REQUEST["SDate"]) ){
    ...
    $input = $_REQUEST["SDate"];
	include '../connect.php';
	if($_REQUEST["hassearch"] != "none"){
		
		$hassearch = $_REQUEST["hassearch"];

		$total_pages_sql = "select * FROM orders where order_code = '".$hassearch."' and date_ordered like '%".$input."%'";
		//echo $Get_query;
	}else{
		$total_pages_sql = "select * FROM orders where date_ordered like '%".$input."%'";
		//echo "nosearch";
	}
	...
	$sql = $total_pages_sql." LIMIT $offset, $no_of_records_per_page";
	...
	if($result->rowCount() > 0){
        ...
        <div class="hint-text">Search Date:  "<?php echo $input ;?>" <b>( <?php echo $result->rowCount() ;?> )</b></div>
        <?php
